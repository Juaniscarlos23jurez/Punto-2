...
public class adaptadorProbedor extends RecyclerView.Adapter<adaptadorProbedor.Myholder>{
    FirebaseUser user;
    FirebaseAuth auth;
    Context context;
    ArrayList<provedor> provedor1;
    public adaptadorProbedor(Context context, ArrayList<provedor> provedor1) {
    ...
    @NonNull
    @Override
    public adaptadorProbedor.Myholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.e_d_provedor,parent,false);
        auth= FirebaseAuth.getInstance();
        user= auth.getCurrentUser();
        return new adaptadorProbedor.Myholder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull adaptadorProbedor.Myholder holder, int position) {
        ...
        DatabaseReference reference1 = database1.getReference("/MI data base Usuarios/" + user.getUid()+"/Provedor"); //nombre de data de base
